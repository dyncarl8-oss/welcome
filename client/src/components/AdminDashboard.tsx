...
export default function AdminDashboard({ userName, experienceId }: AdminDashboardProps) {
  ...
  const toggleAutomationMutation = useMutation({
    mutationFn: async (isActive: boolean) => {
      const res = await apiRequest("POST", "/api/admin/toggle-automation", { isActive });
      return await res.json();
    },
    onSuccess: (data) => {
      queryClient.invalidateQueries({ queryKey: ["/api/admin/creator"] });
  ...
  const triggerAudioMutation = useMutation({
    mutationFn: async (customerId: string) => {
      const res = await apiRequest("POST", "/api/admin/trigger-audio", { customerId });
      return await res.json();
    },
    onSuccess: (data) => {
                            ...
                            <div className="flex-1 min-w-0 space-y-3">
                              <div>
                                <div className="flex items-center gap-2 flex-wrap mb-1">
                                  ...
                                  {member.plan?.name && (
                                    <Badge variant="secondary" data-testid={`badge-member-plan-${index}`}>
                                      {member.plan.name}
                                    </Badge>
                                  )}
                                  {member.status && (
                              ...
                              <div className="grid grid-cols-2 gap-3 text-sm">
                                ...
                                {member.joined_at && (
                                  <div className="flex items-center gap-2" data-testid={`text-member-joined-${index}`}>
                                    ...
                                    <span>
                                      Joined {(() => {
                                        const date = typeof member.joined_at === 'number' 
                                          ? new Date(member.joined_at * 1000) 
