...
async function reconcileMembership(creator: Creator): Promise<{ isCancelled: boolean; shouldDowngrade: boolean }> {
  try {
    ...
    const ourPlanIds = ["plan_28isMowaQAjxK", "plan_ALL5Dj5k7Ps54"];
    ...
    if (shouldUpgrade) {
      ...
      if (planId === "plan_28isMowaQAjxK") {
        planType = "tier200";
        credits = 200;
      } else if (planId === "plan_ALL5Dj5k7Ps54") {
        planType = "unlimited";
        credits = 999999;
      }
  ...
  app.get("/api/admin/credits", async (req, res) => {
    try {
      ...
      const PLAN_DETAILS = {
        ...
        tier200: { name: "Pro Plan", limit: 200, price: "$29/mo", planId: "plan_28isMowaQAjxK" },
        unlimited: { name: "Unlimited Plan", limit: null, price: "$99/mo", planId: "plan_ALL5Dj5k7Ps54" }
      };
